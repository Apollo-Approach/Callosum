...
HOME = Path.home()
LUMI_DIR = Path(os.environ.get("Callosum_SOURCE_DIR", str(HOME / "Desktop/transcripts")))

# People we know about (for name detection in content)
# Loaded from ~/.callosum/known_names.json if it exists, otherwise generic fallback.
_KNOWN_NAMES_PATH = HOME / ".callosum" / "known_names.json"
_FALLBACK_KNOWN_PEOPLE = ["Alice", "Ben", "Riley", "Max", "Sam", "John", "Jordan"]
...
def extract_people(lines):
    ...
    if dir_match:
        ...
        # User can map usernames to names in ~/.callosum/known_names.json
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Source directory (default: Callosum_SOURCE_DIR or ~/Desktop/transcripts)",
    )
    parser.add_argument(
