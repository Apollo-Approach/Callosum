"""
palace_graph.py -- Graph traversal layer for Callosum
======================================================
...
def traverse(start_room: str, col=None, config=None, max_hops: int = 2):
    ...
    Respects wing isolation: only follows connections to wings that
    are explicitly linked via tunnel_links in wing_config.json.
    ...
    # The starting wings establish our isolation boundary
    origin_wings = set(start["wings"])
    ...
    # BFS traversal with isolation guard
    ...
    while frontier:
        ...
        for room, data in nodes.items():
            ...
            # ISOLATION GUARD: only allow traversal if at least one
            # shared wing is linked to one of our origin wings
            allowed_shared = set()
            for sw in shared_wings:
                for ow in origin_wings:
                    if is_linked(ow, sw):
                        allowed_shared.add(sw)
                        break
                # Also allow same-wing (intra-wing traversal)
                if sw in origin_wings:
                    allowed_shared.add(sw)
            ...
            results.append(
                {
                    ...
                    "count": data["count"],
                    "hop": depth + 1,
                    "connected_via": sorted(allowed_shared),
                }
            )
            if depth + 1 < max_hops:
...
def find_tunnels(wing_a: str = None, wing_b: str = None, col=None, config=None):
    """
    Find rooms that connect two wings (or all linked tunnel rooms).
    These are the "hallways" -- same named idea appearing in multiple domains.

    Respects wing isolation: only shows tunnels between explicitly linked wings.
    """
    # If both wings specified, check they're linked first
    if wing_a and wing_b and not is_linked(wing_a, wing_b):
        return {
            "error": f"Wings not linked: {wing_a} <-> {wing_b}",
            "hint": "Run Callosum_link_wings to explicitly allow cross-wing tunnels",
        }
    ...
    for room, data in nodes.items():
        ...
        # ISOLATION GUARD: only include tunnels where at least one
        # wing pair is linked
        has_linked_pair = False
        for i, wa in enumerate(wings):
            for wb in wings[i + 1 :]:
                if is_linked(wa, wb):
                    has_linked_pair = True
                    break
            if has_linked_pair:
                break

        if not has_linked_pair:
            continue
