...
Used by Callosum init before mining begins.
...
def _print_entity_list(entities: list, label: str):
    ...
    for i, e in enumerate(entities):
        ...
        try:
            ...
            # Fallback for emojis or strange characters in the name/signals
            safe_name = e["name"].encode("ascii", "replace").decode("ascii")
            safe_signals = signals_str.encode("ascii", "replace").decode("ascii")
            print(f"    {i + 1:2}. {safe_name:20} [{confidence_bar}] {safe_signals}")
...
def confirm_entities(detected: dict, yes: bool = False) -> dict:
    ...
    """
    print(f"\n{'=' * 58}")
    print("  Callosum -- Entity Detection")
    print(f"{'=' * 58}")
