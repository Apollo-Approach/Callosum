#!/usr/bin/env python3
"""
onboarding.py -- Callosum first-run setup.

Asks the user:
  1. How they're using Callosum (work / personal / combo)
...
Seeds the entity_registry with confirmed data so Callosum knows your world
from minute one -- before a single session is indexed.

Usage:
    python3 -m Callosum.onboarding
    or: Callosum init
"""

from pathlib import Path
from callosum.entity_registry import EntityRegistry
from callosum.entity_detector import detect_entities, scan_for_detection
...
def _ask_mode() -> str:
    _header("Welcome to Callosum")
    print("""
  Callosum is a personal memory system. To work well, it needs to know
...
""")
    print("  How are you using Callosum?")
    print()
...
def _ask_people(mode: str) -> tuple[list, dict]:
  ...
  Format: name, relationship (e.g. "Riley, daughter" or just "Your Name")
...
def _ask_projects(mode: str) -> list:
    ...
    _hr()
    print("""
  What are your main projects? (These help Callosum distinguish project
...
def _warn_ambiguous(people: list) -> list:
    ...
    """
    from callosum.entity_registry import COMMON_ENGLISH_WORDS
...
def _generate_aaak_bootstrap(
    ...
    """
    Callosum_dir = Path(config_dir) if config_dir else Path.home() / ".callosum"
    Callosum_dir.mkdir(parents=True, exist_ok=True)
    ...
    registry_lines = [
        "# AAAK Entity Registry",
        "# Auto-generated by Callosum init. Update as needed.",
        "",
        "## People",
    ]
    for p in people:
    ...
    (Callosum_dir / "aaak_entities.md").write_text("\n".join(registry_lines))
    ...
    (Callosum_dir / "critical_facts.md").write_text("\n".join(facts_lines))
...
def run_onboarding(
  ...
  Callosum will check the context before treating them as person names.
...
""")
    ...
    print("\n  AAAK entity registry: ~/.callosum/aaak_entities.md")
    print("  Critical facts bootstrap: ~/.callosum/critical_facts.md")
